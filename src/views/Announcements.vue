            ...
            <div class="d-flex align-items-center gap-3">
              ...
              <div>
                ...
                <p class="mb-0" style="font-size: 0.75rem; color: var(--dark-light);">
                  {{ editing ? 'Update the announcement details below' : 'Fill in the details to publish a new announcement' }}
                </p>
              </div>
            </div>
                  ...
                  <div class="form-section-body">
                    <div class="form-floating mb-3">
                      <input
                        ...
                        @input="formError = ''"
                      />
                      <label for="newsTitle">Announcement Title</label>
                    ...
                    <div class="content-editor-wrapper">
                      <div class="content-editor-toolbar">
                        <span class="toolbar-label">Write your announcement</span>
                      ...
                      <div v-else class="content-preview">
                        <h4 class="preview-title">{{ form.title || 'Untitled Announcement' }}</h4>
                        <hr class="church-divider-solid" />
      ...
      <div class="modal-overlay" v-if="showDeleteModal" @click.self="showDeleteModal = false">
        <div class="church-card modal-card modal-card-sm">
          <div class="p-4 text-center">
            ...
            <h5 style="color: var(--burgundy); font-family: var(--font-heading);">Delete Announcement</h5>
            <p class="text-muted mb-4">Are you sure you want to delete this announcement?</p>
...
const recentPosts = computed(() =>
  aStore.announcements.slice(0, 5).map((a) => ({ id: a.id, title: a.title, date: a.startPublishDate || a.date, startPublishDate: a.startPublishDate, endPublishDate: a.endPublishDate }))
);
...
async function handleSave() {
  ...
  try {
    if (editing.value && editingId.value) {
      await aStore.updateAnnouncement(editingId.value, payload);
    } else {
      await aStore.createAnnouncement(payload);
    }
...
async function handleDelete() {
  ...
  try {
    await aStore.deleteAnnouncement(editingId.value);
    showDeleteModal.value = false;
    await aStore.fetchAnnouncements();
  } catch {
    formError.value = "Failed to delete.";
  }
}

async function handleLike(article) {
  if (!userStore.uid) return;
  await aStore.toggleLike(article.id, userStore.uid, userStore.displayName || "Anonymous");
  await aStore.fetchAnnouncements();
}

async function handleAddComment(article) {
  if (!commentText.value.trim() || !userStore.uid) return;
  await aStore.addComment(article.id, userStore.uid, userStore.displayName || userStore.email || "Anonymous", commentText.value.trim());
  commentText.value = "";
  await aStore.fetchAnnouncements();
}
